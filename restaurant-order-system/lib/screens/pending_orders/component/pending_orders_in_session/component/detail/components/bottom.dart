  ...
  Widget build(BuildContext context) {
    ...
    void _handleSubmitted(context) async {
      ...
      if (reason.trim() == "" || reason == null) {
        ...
        var isSuccess = await MyApi().declineOrder(order.orderId, reason);
        ...
        if (isSuccess) {
          ...
          await DialogUtil()
              .errorDialog(context, () {}, "ERROR", "Order declined fail");
        }
      }
    }
                    ...
                    press: () async {
                      if (order != null) {
                        DialogUtil().confirmDialog(context, () async {
                          var isSuccess =
                              await MyApi().confirmOrder(order.orderId);
                          if (isSuccess) {
                            await DialogUtil().successDialog(context, () {
                              Navigator.pop(context);
                            }, "SUCCESS", "Order confirmed successfuly");
                          } else {
                            await DialogUtil().errorDialog(context, () {},
                                "ERROR", "Order confirmed fail");
                          }
